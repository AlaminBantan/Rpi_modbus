...
ax = sns.boxplot(x=df['Date'].dt.date, y='Average fruit weight', hue='Treatment', data=df, palette=custom_palette)
plt.title('Average Fruit Weight per Plant')
plt.xlabel('Date')
plt.ylabel('Average Fruit Weight')
...
output_filename = "sample_average_fruit_weight.svg"
