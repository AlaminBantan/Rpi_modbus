...
ax = sns.boxplot(x=df['Date'].dt.date, y='Leaf length 5 (cm)', hue='Treatment', data=df, palette=custom_palette)
plt.title('5th leaf length in Zone C')
plt.xlabel('Date')
plt.ylabel('5th leaf length (cm)')
...
output_filename = "5th _leaf_length_zone_C.svg"
