...
ax = sns.boxplot(x=df['Date'].dt.date, y='IN Length 5 (cm)', hue='Treatment', data=df, palette=custom_palette)
plt.title('Internode distance to 5th leaf in Zone C')
plt.xlabel('Date')
plt.ylabel('Internode distance to 5th leaf (cm)')
...
output_filename = "5th_leaf_zone_C.svg"
