...
csv_path = r'C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Data collection\Climatic_data\mean_Temperature_data.csv'
...
for date, group_data in zone_b_grouped.groupby('date'):
    ...
    plt.title(f'Daily Temperature Zone B - {date}')
    plt.xlabel('Time')
    ...
    file_path = r'C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Data collection\Climatic_data\plots\Temp\\' + file_name
...
for date, group_data in zone_c_grouped.groupby('date'):
    ...
    plt.title(f'Daily Temperature Zone C - {date}')
    plt.xlabel('Time')
    plt.ylabel('Temperature (C)')
