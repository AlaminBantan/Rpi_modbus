...
csv_path = r'C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Data collection\Climatic_data\mean_SolarRadiation_data.csv'
...
for date, group_data in zone_b_grouped.groupby('date'):
    ...
    plt.title(f'Daily solar radiation Zone B - {date}')
    plt.xlabel('Time')
    ...
    file_name = f'Daily_solar_Zone_B_{date}.png'
    file_path = r'C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Data collection\Climatic_data\plots\Solar radiation\\' + file_name
...
for date, group_data in zone_c_grouped.groupby('date'):
    ...
    plt.title(f'Daily solar radiation Zone C - {date}')
    plt.xlabel('Time')
    plt.ylabel('Solar radiation (w.m-2)')
    ...
    file_name = f'Daily_solar_Zone_C_{date}.png'
    file_path = r'C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Data collection\Climatic_data\plots\Solar radiation\\' + file_name
    plt.savefig(file_path)
    plt.close()
